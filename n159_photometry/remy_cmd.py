...
c160 = SkyCoord(ra=ir_df['ra_f160w'] * u.deg, dec=ir_df['dec_f160w'] * u.deg)

max_sep = 0.2* u.arcsec
idx, d2d, d3d = c814.match_to_catalog_sky(c160)
sep_constraint = d2d < max_sep
print(np.sum(sep_constraint))
c814_matches = vi_df[sep_constraint]
c160_matches = ir_df.iloc[idx[sep_constraint]]
cmatch_irvi = c160_matches.join(c814_matches)

cmatch_irvi.to_csv(savephotdir+region+'_reduce_visir.phot.csv',index=False)
...
plt.figure()
#plt.scatter(vi_df['555min814'],vi_df['mag_555'],s=0.4,label='All')
...
plt.figure()
#plt.scatter(ir_df['125min160'],ir_df['mag_125'],s=0.4,label='All')
